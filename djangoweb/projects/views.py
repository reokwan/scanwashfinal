...
def video_feed(request, camera_id):
    global caps
    if not caps[camera_id] or not caps[camera_id].isOpened():
...
def start_detection_view(request):
    user = request.user
    config = CameraConfig.objects.filter(user=user).first()
    if not config:
        return HttpResponse("Camera configuration not found.", status=400)
    
    camera_urls = [config.camera1_url, config.camera2_url, config.camera3_url]
    start_detection(camera_urls)
